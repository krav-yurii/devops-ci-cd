...
### argo_cd
Встановлює Argo CD через Helm у namespace `argocd`. Автоматично реєструє `Application`, що стежить за гілкою `final-project` та шляхом `charts/django-app`, і синхронізує зміни у кластер (`prune: true`, `selfHeal: true`).
...
module "rds" {
  ...
  tags = {
    ...
    Project     = "krav-yurii-final-project"
  }
}
```
...
[Git push] → [Jenkins pipeline]
                ...
                └── git: update charts/django-app/values.yaml → push to final-project
                                          ↓
...
# Перевизначити паролі (за замовчуванням KravYurii*):
terraform apply \
...
aws eks update-kubeconfig --region eu-north-1 --name krav-yurii-final-project-eks
...
aws ecr list-images --repository-name krav-yurii-final-project-ecr --region eu-north-1
...
4. На головній сторінці побачите Application **django-app**:
   ...
   - **Revision**: поточний коміт гілки `final-project`.
